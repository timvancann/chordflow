...
mod tests {
    ...
    fn test_tick() {
        ...
        let mut metronome = Metronome::new(100, 2, 4, time_source);
        metronome.start();

        metronome.beat_timer.time_source = Box::new(move || fake_time + Duration::from_millis(500));
        metronome.tick();
        assert_eq!(metronome.current_bar, 0);
        assert_eq!(metronome.current_beat, 0);

        metronome.beat_timer.time_source = Box::new(move || fake_time + Duration::from_millis(600));
        metronome.tick();
        assert_eq!(metronome.current_bar, 0);
        assert_eq!(metronome.current_beat, 1);

        metronome.beat_timer.time_source =
            Box::new(move || fake_time + Duration::from_millis(2400));
        metronome.tick();
        assert_eq!(metronome.current_bar, 1);
        assert_eq!(metronome.current_beat, 0);

        metronome.beat_timer.time_source =
            Box::new(move || fake_time + Duration::from_millis(3000));
        metronome.tick();
        assert_eq!(metronome.current_bar, 1);
        assert_eq!(metronome.current_beat, 1);

        metronome.beat_timer.time_source =
            Box::new(move || fake_time + Duration::from_millis(4800));
        metronome.tick();

        assert!(metronome.has_cycle_ended());
        assert_eq!(metronome.current_bar, 0);
        assert_eq!(metronome.current_beat, 0);
    }
}
